...
export const generateDefaultState = (): NourOSState => {
  ...
  // Seed deep work sessions & matching time tracking entries
  ...
  const timeEntries = [
    {
      ...
      note: 'Completed memory allocation test suite.',
      createdAt: Date.now() - 3600000 * 2,
      startedAt: Date.now() - 3600000 * 3.5,
  ...
  for (let i = 1; i <= 20; i++) {
    ...
    if (i % 2 === 0) {
      const isFastAPI = i % 4 === 0;
      const focus = isFastAPI ? 'FastAPI Microservice Engine' : 'Python Algorithms & Data Structures';
      const area = isFastAPI ? 'Career' : 'Learning';
      const category = isFastAPI ? 'FastAPI' : 'Python';
      ...
      timeEntries.push({
        ...
        source: 'timer' as const,
        status: 'completed' as const,
        note: 'Deep uninterrupted session.',
        createdAt: pastDate.getTime() + 14 * 3600000,
        startedAt: pastDate.getTime() + 12 * 3600000,
        endedAt: pastDate.getTime() + 14 * 3600000
      });
    }
  }
...
export const loadState = (): NourOSState => {
  try {
    ...
    // Refresh / crash recovery for active timer
    if (parsed.activeDeepWork && parsed.activeDeepWork.isRunning) {
      const now = Date.now();
      if (!parsed.activeDeepWork.isPaused) {
        const last = parsed.activeDeepWork.lastTickTimestamp || parsed.activeDeepWork.startedAt || now;
        const elapsedSinceLast = Math.max(0, Math.floor((now - last) / 1000));
        // Cap reload delta to max 8 hours in case browser was closed overnight
        const safeDelta = Math.min(elapsedSinceLast, 8 * 3600);
        parsed.activeDeepWork.elapsedSeconds = (parsed.activeDeepWork.elapsedSeconds || 0) + safeDelta;
        parsed.activeDeepWork.lastTickTimestamp = now;
      }
    }

    // Guarantee timeEntries exists and backfill from deepWorkSessions if empty
    if (!Array.isArray(parsed.timeEntries) || parsed.timeEntries.length === 0) {
      if (Array.isArray(parsed.deepWorkSessions) && parsed.deepWorkSessions.length > 0) {
        parsed.timeEntries = parsed.deepWorkSessions.map((s: any) => ({
          id: `time_dw_${s.id}`,
          date: s.date,
          durationMinutes: s.durationMinutes,
          durationSeconds: s.durationMinutes * 60,
          area: (s.focusArea && s.focusArea.toLowerCase().includes('fastapi')) ? 'Career' : 'Learning',
          category: (s.focusArea && s.focusArea.toLowerCase().includes('fastapi')) ? 'FastAPI' : 'Python',
          source: 'timer',
          status: 'completed',
          note: s.notes,
          createdAt: s.timestamp || Date.now(),
          startedAt: (s.timestamp || Date.now()) - s.durationMinutes * 60000,
          endedAt: s.timestamp || Date.now()
        }));
      } else {
        parsed.timeEntries = [];
      }
